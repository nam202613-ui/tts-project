...
export default function App() {
  ...
  const speakAzure = async () => {
    ...
    setIsSpeaking(true);
    const parts = text.split(/(\[pause[:=]?\s*\d*\.?\d+s?\])/i);
    ...
    try {
      for (let part of parts) {
        ...
        if (pauseMatch) {
          await new Promise(r => setTimeout(resolve, parseFloat(pauseMatch[1]) * 1000));
          continue;
        }

        const p = Math.floor((pitch - 1) * 50);
        const s = Math.floor((speed - 1) * 100);
        
        // HỆ THỐNG DỰ PHÒNG 2 SERVER
        const server1 = `https://api.vncool.com/api/tts?text=${encodeURIComponent(part)}&voice=${selectedVoice}&speed=${s}&pitch=${p}`;
        const server2 = `https://api.ttsonline.top/api/tts?text=${encodeURIComponent(part)}&voice=${selectedVoice}&speed=${s}&pitch=${p}`;

        await new Promise((resolve, reject) => {
          const audio = new Audio(server1);
          audio.volume = volume;
          audioRef.current = audio;
          audio.onended = resolve;
          audio.onerror = () => {
            // Nếu server 1 lỗi, thử server 2
            const fallbackAudio = new Audio(server2);
            fallbackAudio.volume = volume;
            audioRef.current = fallbackAudio;
            fallbackAudio.onended = resolve;
            fallbackAudio.onerror = () => reject("Cả 2 server đều bận");
      ...
      alert("Hệ thống đang quá tải, bro thử lại sau 5 giây nhé!");
    } finally {
      setIsSpeaking(false);
    }
  };
  ...
  return (
    <div className="flex h-screen w-full relative overflow-hidden bg-black text-white font-sans">
      <div className="absolute inset-0 z-0" style={{ backgroundImage: `url('https://files.catbox.moe/lovccc.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
      ...
      <aside className={cn("relative z-20 h-full bg-zinc-950/90 backdrop-blur-2xl border-r border-white/5 w-72 flex flex-col transition-all duration-300", !sidebarOpen && "-ml-72")}>
        <div className="p-6 border-b border-white/5 flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-black italic tracking-tighter">EasyVoice</h1><p className="text-[10px] uppercase text-indigo-400 font-bold">Phòng thu Studio</p></div>
        </div>
        <nav className="p-4"><div className="px-4 py-4 bg-indigo-600/20 text-indigo-300 border border-indigo-500/30 rounded-2xl text-sm font-bold flex items-center gap-3"><Mic2 size={18} /> Giọng Người Thật</div></nav>
      </aside>

      <div className="flex-1 flex flex-col relative z-10">
        <header className="h-16 border-b border-white/5 bg-black/40 flex items-center px-8 justify-between">
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="bg-zinc-950/90 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative">
                ...
                <div className="mt-8 flex items-center gap-4 border-t border-white/5 pt-6">
                  <div className="flex-1 flex gap-2">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center mr-2">Chèn nghỉ:</span>
                    {[0.5, 1, 2].map(s => ( <button key={s} onClick={() => insertPause(s)} className="px-4 py-2 bg-zinc-900 hover:bg-indigo-600 border border-white/10 rounded-lg text-xs font-bold transition-all">+{s}s</button> ))}
                  </div>
                  <button onClick={() => setText('')} className="p-3 text-zinc-600 hover:text-red-400 transition-all"><Trash2 size={24} /></button>
                </div>
              </div>

              {/* LỊCH SỬ DƯỚI KHUNG NHẬP */}
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-zinc-500 px-2 text-[10px] font-black uppercase tracking-widest"><Clock size={12} /> Lịch sử sáng tạo</div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-zinc-950/90 border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-xl">
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-2 uppercase"><span>Tốc độ</span><span className="text-indigo-400 font-black">{speed}x</span></div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-2 uppercase"><span>Cao độ</span><span className="text-indigo-400 font-black">{pitch}</span></div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-2 uppercase"><span>Âm lượng</span><span className="text-indigo-400 font-black">{Math.round(volume * 100)}%</span></div>
              ...
              {/* NÚT TẢI VÀ PHÁT */}
              <button onClick={() => window.open(`https://api.vncool.com/api/tts?text=${encodeURIComponent(text.slice(0,500))}&voice=${selectedVoice}&download=1`)} className="w-full py-4 bg-zinc-900 border border-white/10 hover:bg-white/5 rounded-2xl flex items-center justify-center gap-3 text-xs font-bold transition-all"><Download size={18} /> TẢI FILE MP3</button>

              <button onClick={isSpeaking ? stopSpeaking : speakAzure} className={cn("w-full py-8 rounded-[2.5rem] text-2xl font-black flex items-center justify-center gap-4 transition-all shadow-2xl active:scale-95", isSpeaking ? "bg-red-600 shadow-red-600/40" : "bg-indigo-600 hover:bg-indigo-500 shadow-indigo-600/40")}>
