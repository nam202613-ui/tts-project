...
export default function App() {
  ...
  // HÀM XỬ LÝ PHÁT GIỌNG AI + LỆNH PAUSE
  const speakAzure = async () => {
    ...
    const segments = text.split(/(\[pause[:=]?\s*\d*\.?\d+s?\])/i);
    
    for (let i = 0; i < segments.length; i++) {
      const segment = segments[i].trim();
      if (!segment) continue;

      // Kiểm tra nếu là lệnh pause
      const pauseMatch = segment.match(/\[pause[:=]?\s*(\d*\.?\d+)s?\]/i);
      if (pauseMatch) {
      ...
      // Nếu là văn bản, gọi API lấy audio
      try {
        const pitchAdjusted = Math.floor((pitch - 1) * 50); // Chuyển đổi pitch sang định dạng API
        const speedAdjusted = Math.floor((speed - 1) * 100);
        
        const url = `https://api.ttsonline.top/api/tts?text=${encodeURIComponent(segment)}&voice=${selectedVoice}&speed=${speedAdjusted}&pitch=${pitchAdjusted}&volume=${Math.floor(volume * 100)}`;
        ...
        console.error("Lỗi phát đoạn:", segment);
      }
    }
  ...
  const stopSpeaking = () => {
    ...
    setIsSpeaking(false);
    window.location.reload(); // Đảm bảo dừng hẳn luồng phát
  };
    ...
    <div className="flex h-screen w-full relative overflow-hidden bg-black text-white">
      ...
      <aside className={cn("relative z-20 h-full bg-zinc-950/90 backdrop-blur-2xl border-r border-white/5 w-72 flex flex-col transition-all duration-300", !sidebarOpen && "-ml-72")}>
        <div className="p-6 border-b border-white/10 flex items-center gap-3">
          <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-500/20"><Play size={24} fill="white" /></div>
          <div><h1 className="text-2xl font-black italic tracking-tighter">EasyVoice</h1><p className="text-[10px] uppercase text-indigo-400 font-bold tracking-widest text-center">Phòng thu Studio</p></div>
        </div>
      ...
      <div className="flex-1 flex flex-col relative z-10">
        <header className="h-16 border-b border-white/5 bg-black/40 backdrop-blur-sm flex items-center px-8 justify-between">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-zinc-800 rounded-lg"><Menu size={28} /></button>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">Live Workspace</span>
          </div>
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              <div className="bg-zinc-950/90 border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative">
                ...
                <div className="mt-8 flex items-center gap-4 border-t border-white/5 pt-6">
                  <div className="flex-1 flex gap-2 overflow-x-auto pb-2">
                    ...
                    {[0.5, 1, 1.5, 2].map(s => (
                      <button key={s} onClick={() => insertPause(s)} className="px-4 py-2 bg-zinc-900 hover:bg-indigo-600 border border-white/10 rounded-xl text-xs font-bold transition-all">+{s}s</button>
                    ))}
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-zinc-500 px-2 uppercase tracking-[0.2em] text-[10px] font-black">
                  <Clock size={12} /> Lịch sử sáng tạo
                </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-zinc-950/90 border border-white/10 rounded-[2rem] p-8 space-y-8 shadow-xl">
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-3 uppercase"><span>Tốc độ</span><span className="text-indigo-400">{speed}x</span></div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-3 uppercase"><span>Cao độ</span><span className="text-indigo-400">{pitch}</span></div>
                ...
                <div>
                  <div className="flex justify-between text-[10px] font-bold text-zinc-500 mb-3 uppercase"><span>Âm lượng</span><span className="text-indigo-400">{Math.round(volume * 100)}%</span></div>
              ...
              {/* NÚT TẢI VÀ NÚT PHÁT */}
              <button onClick={() => window.open(`https://api.ttsonline.top/api/tts?text=${encodeURIComponent(text.slice(0,200))}&voice=${selectedVoice}&speed=${Math.floor((speed-1)*100)}&pitch=${Math.floor((pitch-1)*50)}&download=1`)} className="w-full py-4 bg-zinc-900 border border-white/10 hover:bg-white/5 rounded-2xl flex items-center justify-center gap-3 text-xs font-bold transition-all"><Download size={18} /> TẢI FILE MP3</button>

              <button onClick={isSpeaking ? stopSpeaking : speakAzure} className={cn("w-full py-8 rounded-[2.5rem] text-2xl font-black flex items-center justify-center gap-4 transition-all shadow-2xl active:scale-95", isSpeaking ? "bg-red-600" : "bg-indigo-600 hover:bg-indigo-500 shadow-indigo-600/40")}>
